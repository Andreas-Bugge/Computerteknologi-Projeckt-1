...
class Obstacle():  
    ...
    def get_scan(self):
        ...
        Front_cone = min(scan_filter[0:20] + scan_filter[345:359])
        Backing_cone = min(scan_filter[170:190])
        Left_cone = min(scan_filter[10:50])
        Right_cone = min(scan_filter[310:350])

        return Front_cone, Backing_cone, Left_cone, Right_cone, Full_scan
    ...
    def obstacle(self):
        ...
        while not rospy.is_shutdown() and time.time() < End_run:
            Front_cone, Backing_cone, Front_Left_cone, Front_Right_cone, _ = self.get_scan()
            ...
            # If turtlebot is NOT moving
                ...
                # Deciding to back untill collision is avoided
                if Front_cone < SAFE_STOP_DISTANCE:
                    while Backing_cone > SAFE_STOP_DISTANCE:
                        Backing(LINEAR_VEL)
                        _, Backing_cone, _, _, _ = self.get_scan()
                    
                    # Obstacle avoided, turtlebot will move forward.
                    turtlebot_moving = True
